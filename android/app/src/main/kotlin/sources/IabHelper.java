...
import sources.util.BroadcastIAB;
import sources.util.IAB;
import sources.util.IABLogger;
import sources.util.IabException;
import sources.util.IabResult;
import sources.util.Inventory;
import sources.util.Purchase;
import sources.util.Security;
import sources.util.ServiceIAB;
import sources.util.SkuDetails;
import sources.util.communication.BillingSupportCommunication;
import sources.util.communication.OnConnectListener;
...
public class IabHelper {
     ...
     * @param storePackageName storePackageName.
     * @param bindURL          Store Bind Url.
     */
    public IabHelper(Context ctx, String storePackageName, String bindURL) {
    ...
    public void startSetup(final OnIabSetupFinishedListener listener) {
        ...
        } else if (TextUtils.isEmpty(bindURL)) {
            throw new NoSuchElementException("marketbindURL not set or empty");
        }

        if (!isMarketInstalled(storePackageName) && listener != null) {
        ...
        IAB serviceIAB = new ServiceIAB(logger, storePackageName, bindURL, mSignatureBase64);
        ...
        if (canConnectToService) {
            ...
            IAB broadcastIAB = new BroadcastIAB(mContext, logger, storePackageName, bindURL, mSignatureBase64);
    ...
    private boolean isMarketInstalled(String storePackageName) {
        try {
            return mContext.getPackageManager().getApplicationInfo(storePackageName, 0) != null;
    ...
    int queryPurchases(Inventory inv, String itemType) throws JSONException, RemoteException {
        ...
        do {
            ...
            for (int i = 0; i < purchaseDataList.size(); ++i) {
                ...
                if (mSignatureBase64 == null || Security.verifyPurchase(mSignatureBase64, purchaseData, signature)) {
